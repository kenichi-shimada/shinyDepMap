...
output$inter_cluster <- renderPlotly({
	...
	if(this.eid==""){
		...
		this.sym.all <- df.0$sym.all[df.0$eid == this.eid]
		this.sym <- df.0$sym[df.0$eid == this.eid]
		this.cl <- df.0[df.0$eid == this.eid,this.mem]

		coef.1 <- data.frame(coef=this.coef,eid=names(this.coef),stringsAsFactors=FALSE) %>%
			left_join(df.0,by="eid") %>%
			filter(eid != this.eid)

		mem <- as.character(coef.1[[this.mem]])
		mem[is.na(mem)] <- "none"
		...
		coef.1 <- coef.1 %>% mutate(mem=factor(mem,levels=clust.levs)) %>%
			mutate(text=paste0(sym,"(",mem,") - ",this.sym,"(",this.cl,")<br />",coef))
		...
		suppressWarnings({ # doesn't work..
			pvio <- ggplot(sum.coef.1, aes(x=mem,y=mids,color=mem)) +
				...
				ggtitle(paste0(this.sym.all, "\t(",this.cs," size clusters)")) + 
