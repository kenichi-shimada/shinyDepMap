...
sliderInput("lik2",h5("Cluster consistency (Likelihood):"),
  min=0,max=1,round=-1,step=.1,value=0)
...
output$gene_subset2 <- renderUI({ ## in response to symbol, get the probe
  ...
  if((!gen.path2 %in% c("cluster","pathway"))||
    ...
    this.mems <- mems %>% 
      filter(eid %in% eids & lik >= lik2) %>%
      arrange(efficacy,desc(selectivity))
    this.set <- this.mems$eid
    names(this.set) <- this.mems$sym
...
igene2 <- reactive({
  if(is.null(input$ess_gen_clust)||
    ...
    ( input$ess_gen_clust %in% c("pathway","cluster") && (is.null(input$eid20)||input$eid20==""))){
      return(NULL)
  }else if(input$ess_gen_clust=="essential gene"){
    this.eid <- input$eid2
  }else if(input$ess_gen_clust %in% c("pathway","cluster")){
...
observeEvent(input$lik2,{
  mem.1 <-  sort(as.numeric(names(which(table(mems$mem[mems$lik >= lik2])>0))))
  syms <- mems$sym.all[mems$lik >= input$lik2 & mems$mem %in% mem.1]
  eids.ess.1 <- eids.ess[syms]
  updatePickerInput(session,
    inputId = "eid2",
    choices = c("",eids.ess.1),
    selected ="")
  updatePickerInput(session,
    inputId = "clust2",
    choices = c("",mem.1),
...
observeEvent(input$select_button, {
  this.mem <<- strsplit(input$select_button, "_")[[1]][2]
  # Method 1
  updateRadioButtons(session,
    inputId="ess_gen_clust",
      choices=c("essential gene","cluster","pathway"),  
      selected = "cluster"
    )
  updatePickerInput(session,
