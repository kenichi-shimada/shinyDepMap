...
output$all_eff_sel <- renderPlotly({
	...
	suppressWarnings(
		p <- ggplot(df,aes(efficacy,selectivity)) +
			...
			geom_point(data=dummy[2,],aes(efficacy,selectivity),col=NA,size=1) # sym.1
	)
...
observeEvent(input$goButton, {
	sym.query <- input$sym.query
	sym.1 <- input$sym.1
	grep.sym.query <- grepl(paste0("^",sym.query),df$sym,ignore.case=T)

	if(any(grep.sym.query)){
		df.1 <- df[grep.sym.query,] %>%
			mutate(text = paste0(sym.query,"<br />eff:",efficacy,", sel:",selectivity))

		# i.sym.1 <- which(grep.sym.query)[1]

		df.2 <- df.1[rep(match(sym.1,df.1$sym),2),]
...
observeEvent(input$sym.1, {
	sym.1 <- input$sym.1
	if(sym.1!="" && any(df$sym==sym.1)){
		i.sym.1 <- which(df$sym==sym.1)
		df.2 <- df[rep(i.sym.1,2),] %>%
