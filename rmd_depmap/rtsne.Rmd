...
output$tsne <- renderPlotly({
	...
	if(is.null(eids)){
		...
		p <- p +
			geom_point(data=(mems.2 %>% mutate(text="")),
				aes(efficacy,selectivity),col="grey50",size=1.2) + # 6th trace, lik < lik2
			geom_point(data=mems.3,aes(efficacy,selectivity),col="red",size=1.2) # 7th trace, lik >= lik2
	}
